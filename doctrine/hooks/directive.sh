...
Every file under `docs/` belongs to a bucket, whatever its extension — images, diagrams, exports, and attachments go in `_assets/`, never loose under `docs/`. Every new document goes inside one of the six — including documents about code, which do not live next to the code. Nothing new lands directly under `docs/`, at the repository root, or in a pre-existing directory outside the doctrine. Four exceptions, and no others: `README.md` at any level (it means "this directory, explained"), the root files an ecosystem fixes by name (`LICENSE`, `CHANGELOG.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `AGENTS.md`, `CLAUDE.md`), anything inside a dot-directory or a vendored/generated tree, and paths the repository lists in `DOCTRINE_ALLOW`.
