...
deny() { echo "coding: refused — $1" >&2; exit 2; }

case "${CODING_CYCLE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
CODING_CPD="${CLAUDE_PROJECT_DIR:-}" \
CODING_CWD="$(pwd -P 2>/dev/null || echo)" \
python3 <<'PY'
import json, os, posixpath, re, shlex, subprocess, sys

def deny(msg):
    sys.stderr.write("coding: refused — %s\n" % msg)
    sys.exit(2)
...
def _coding_fail_closed(_t, _v, _tb):
    try:
        sys.stderr.write("coding: refused — fail-closed: internal error (%s: %s)\n" % (_t.__name__, _v))
    except Exception:
        pass
    os._exit(2)
sys.excepthook = _coding_fail_closed

raw = os.environ.get("CODING_PAYLOAD", "")
try:
...
cpd = os.environ.get("CODING_CPD", "")
cwd = os.environ.get("CODING_CWD", "") or "."
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "coding: refused — fail-closed: internal error (gate judge exited $rc)" >&2
  exit 2
fi
exit "$rc"
