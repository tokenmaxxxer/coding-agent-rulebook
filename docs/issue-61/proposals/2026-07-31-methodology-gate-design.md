...
- Merging `proposal-shape`/`record-shape`/`survey-order` back into
  ...
  consolidation.
- A fourth plugin for implementation-domain review/test discipline
  (code-review checklist, test-derivation/test-depth norms): reviewed per
  the approver's second FEEDBACK and not added here — no prior maturation
  round in this repo has adopted a concrete review/test-discipline norm
  for this role the way issue-52 adopted the proposal/record shapes;
  there is no ratified methodology text to encode into a gate. Adding one
  now would mean inventing the norm inside an enforcement-design issue,
  which is out of #61's scope (#61 mechanizes what was already adopted,
  it does not adopt new norms). If a future maturation issue ratifies a
  review/test-discipline norm for this role, it becomes a fourth
  independent plugin under this same set, not a retrofit into the three
  above.
